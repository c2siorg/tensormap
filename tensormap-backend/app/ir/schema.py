...
class IRGraph(BaseModel):
    ...
    def to_reactflow_json(self) -> dict:
        ...
        edges = []
        for n in self.nodes:
            pos = getattr(n, "position", {"x": 0, "y": 0})
            ...
            nodes.append(
                {
                    ...
                    "position": pos,
                    "data": {"name": getattr(n, "name", layer_type.title()), "params": params},
                }
            )
        for e in self.edges:
            edges.append(
                {
                    ...
                    "source": e.source_id,
                    "target": e.target_id,
                    "sourceHandle": getattr(e, "source_handle", None),
                    "targetHandle": getattr(e, "target_handle", None),
                }
            )
