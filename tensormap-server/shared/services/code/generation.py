...
def code_generation(code_params):
    ...
    data = {
        ...
        LEARNING_MODEL: {
            MODEL_JSON_FILE: helper_generate_json_model_file_location(
    ...
    template = template_env.get_template(
        helper_map_correct_code_template(problem_type_id=code_params[PROBLEM_TYPE])
    )

    output = template.render(data=data)
    generated_code_file = open(
        CODE_GENERATION_LOCATION + model[MODEL_NAME] + CODE_GENERATION_TYPE, "w+"
    )
...
def generate_code(model_name):
    ...
    data = {
        DATASET: {
            FILE_NAME: helper_generate_file_location(
                file_id=getattr(model_configs, FILE_ID)
            ),
    ...
    template_env = Environment(loader=template_loader)
    template = template_env.get_template(
        helper_map_correct_code_template(problem_type_id=model_configs.model_type)
    )
...
def helper_map_correct_code_template(problem_type_id):
    options = {
        1: CODE_TEMPLATE_FOLDER + "multi-class-all-float-classification-csv.py",
...
def helper_generate_file_location(file_id):
    ...
    return (
        configs["api"]["upload"]["folder"] + "/" + file.file_name + "." + file.file_type
    )
