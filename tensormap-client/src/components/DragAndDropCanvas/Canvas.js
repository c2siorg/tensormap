...
import DropoutNode from './CustomNodes/DropoutNode/DropoutNode.js'; // Import Drop Node component
...
const nodeTypes = {
  ...
  customdropout: DropoutNode // Add Drop Node to node types
};
...
const Canvas = () => {
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      } else if (type === 'customdropout') {
        newNode = {
          id: getId(),
          type,
          position,
          data: { label: `${type} node`, params: { "dropoutRate": '' } },
        };
      } else {
        newNode = {
          id: getId(),
