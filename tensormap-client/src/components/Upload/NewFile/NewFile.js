...
class NewFile extends Component {
  ...
  componentDidMount() {
    this.setState({
      fileType: '', fileName: '', file: null, uploadButtonDisabledStatus: true,
    });
  }
  ...
  fileChange = (e) => {
    this.setState(
      { ...this.state, file: e.target.files[0], fileName: e.target.files[0].name },
      () => {
        this.submitButtonEnableHandler();
      },
    );
        ...
        <Form size="big" onSubmit={this.fileUploadHandler}>
          ...
          <Form.Field>
            <Button
              ...
              size="large"
              type="submit"
              disabled={this.state.uploadButtonDisabledStatus}
            >
