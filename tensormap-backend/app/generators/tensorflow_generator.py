...
class TensorFlowGenerator:
    ...
    Example:
        >>> from app.ir.schema import IRGraph, IRNode, IREdge, InputParams, DenseParams
        >>> graph = IRGraph(
        ...     nodes=[
    ...
    def build_model(self, graph: IRGraph):  # type: ignore
        ...
        # Output nodes are those with no outgoing edges, excluding input nodes
        input_node_ids = {n.id for n in input_nodes}
        output_node_ids = [nid for nid in nodes_by_id if nid not in source_to_targets and nid not in input_node_ids]
