...
function HeatMapComp(props) {
  ...
  const data = xLabels.map((x) => Object.keys(props.corrMatrix[x]).map((y) => props.corrMatrix[x][y]));
  return (
    <div>
      <Header size="medium" textAlign="center" style={{ marginTop: '20px' }}>Co-relation Matrix</Header>
      <div style={{
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontSize: '10px', lineHeight: '1.8',
      }}
      >
        <HeatMap
          xLabels={xLabels}
