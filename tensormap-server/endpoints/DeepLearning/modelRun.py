...
class CustomProgressBar(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        try:
            ...
            raise InternalServerError(description=f"An error occurred during epoch progress update: {e}")
    ...
    def on_batch_end(self, batch, logs=None):
        try:
            ...
            raise InternalServerError(description=f"An error occurred during batch progress update: {e}")

    def on_test_begin(self, logs=None):
        try:
            model_result("Evaluating...")
        except Exception as e:
            raise InternalServerError(description=f"An error occurred at the beginning of testing: {e}")
    ...
    def on_test_end(self, logs=None):
        try:
            ...
            raise InternalServerError(description=f"An error occurred at the end of testing: {e}")
...
def model_result(message):
    try:
        ...
        raise InternalServerError(description=f"An error occurred during emitting model result: {e}")
...
def helper_generate_file_location(file_id):
    try:
        ...
        raise InternalServerError(description=f"An error occurred while generating file location: {e}")

def helper_generate_json_model_file_location(model_name):
    try:
        return MODEL_GENERATION_LOCATION + model_name + MODEL_GENERATION_TYPE
    except Exception as e:
        raise InternalServerError(description=f"An error occurred while generating JSON model file location: {e}")
...
def model_run(incoming):
    try:
        ...
        raise InternalServerError(description=f"An error occurred during model running: {e}")
