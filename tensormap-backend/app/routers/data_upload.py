...
async def upload_file(
    ...
    if ext not in ALLOWED_EXTENSIONS:
        return JSONResponse(
            ...
            content={
                "success": False,
                "message": "Please check the file type. you added wrong format. (CSV, ZIP only)",
                "data": None,
            },
        )
...
async def get_all_files(project_id: uuid_pkg.UUID | None = Query(None), db: Session = Depends(get_db)):
    body, status_code = get_all_files_service(db, project_id=project_id)
    return JSONResponse(status_code=status_code, content=body)
