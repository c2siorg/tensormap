...
class CustomProgressBar(tf.keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        model_result(
            f"{batch + 1}/{self.params['steps']}  [{arrow}{spaces}] {int(progress * 100)}% - Loss: {logs['loss']:.4f} - {metric}",
            1,
        )
...
def model_run(incoming):
    ...
    if model_configs.model_type == 3:
        image_properties = ImageProperties.query.filter_by(
            id=getattr(model_configs, FILE_ID)
        ).first()
        ...
        directory = helper_generate_file_location(
    ...
    json_string = json.dumps(
        yaml.load(open(helper_generate_json_model_file_location(model_name=model_name)))
    )
    ...
    if model_configs.model_type == 3:
        model.fit(
            ...
            epochs=getattr(model_configs, MODEL_EPOCHS),
