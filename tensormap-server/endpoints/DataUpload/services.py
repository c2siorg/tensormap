...
def add_file_service():
    ...
    if filename.endswith(".zip"):
        try:
            ...
            return generic_response(
                status_code=201, success=True, message="File processed successfully"
            )
        except Exception:
    ...
    save_one_record(record=data)
    return generic_response(
        status_code=201, success=True, message="File saved successfully"
    )
    ...
    try:
        ...
        for file in files:
            if file.file_type == "zip":
                ...
                df = pd.read_csv(
                    upload_folder + "/" + file.file_name + "." + file.file_type
                )
    ...
    try:
        ...
        if file:
            file_path = os.path.join(
                upload_folder, f"{file.file_name}.{file.file_type}"
            )
            if file.file_type == "zip":
                ...
                if os.path.isdir(directory_path):
                    ...
                    return generic_response(
                        status_code=200,
                        success=True,
                        message="Zip file and directory deleted successfully",
                    )
                else:
                    return generic_response(
                        status_code=400, success=False, message="Zip file not found"
                    )
            else:
                if os.path.isfile(file_path):
                    ...
                    return generic_response(
                        status_code=400, success=False, message="File not found"
                    )
        else:
            return generic_response(
                status_code=400, success=False, message="File not in the DB"
            )
    except Exception as e:
        # Log the error message and return a generic response
        print(f"An error occurred: {str(e)}")
