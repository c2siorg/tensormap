...
def model_validate_service(incoming):
    try:
        ...
        generated_model_file = open(MODEL_GENERATION_LOCATION + incoming[CODE][DL_MODEL][MODEL_NAME] +
                                    MODEL_GENERATION_TYPE, 'w+')
        ...
        return generic_response(status_code=500, success=False, message=f"Error in model validation: {str(e)}")
...
def get_code_service(incoming):
    try:
        ...
        return generic_response(status_code=500, success=False, message=f"Error in getting code: {str(e)}")
...
def get_available_model_list():
    try:
        ...
        data = [model.model_name for model in model_list]
        return generic_response(status_code=200, success=True, message='Model list generated successfully.', data=data)
    except Exception as e:
        return generic_response(status_code=500, success=False, message=f"Error in getting model list: {str(e)}")
