...
def add_target_service(incoming):
    ...
    try:
        ...
        return generic_response(
            status_code=500, success=False, message=f"Error storing record: {str(e)}"
        )
...
def delete_one_target_by_id_service(file_id):
    if DataFile.query.filter_by(id=file_id).count() > 0:
        if DataProcess.query.filter_by(file_id=file_id).count() > 0:
            ...
            return generic_response(
                status_code=400, success=False, message="Target field doesn't exist"
            )
    else:
        return generic_response(
            status_code=400, success=False, message="File doesn't exist in DB"
...
def get_data_metrics(file_id):
    ...
    if file:
        ...
        return generic_response(
            status_code=200,
            success=True,
            message="Dataset metrics generated succesfully",
            data=metrics,
        )
    else:
        return generic_response(
...
def get_file_data(file_id):
    ...
    if file:
        ...
        return generic_response(
            status_code=400, success=False, message="Unable to open file"
        )
        ...
        try:
            ...
            for transformation in transformations:
                ...
                if transformation["transformation"] == "Categorical to Numerical":
                    df[transformation["feature"]] = pd.Categorical(
                        df[transformation["feature"]]
                    ).codes
            ...
            return generic_response(
                status_code=200,
                success=True,
                message="Preprocessed Dataset created with name: " + file_name,
            )
        except Exception as e:
            return generic_response(
                status_code=500,
...
def update_image_properties(
    file_id, file_type, image_size, batch_size, color_mode, label_mode
):
    try:
        ...
        if image_properties:
            ...
            return generic_response(
                status_code=201,
                success=True,
                message="Image properties added successfully",
            )
    except Exception as e:
        ...
        return generic_response(
            status_code=500,
            success=False,
            message="An error occurred while upserting the image properties",
        )
