...
def model_generation(model_params):
    ...
    while queue:
        ...
        adjacent_targets = [
            node for node in model_params["nodes"] if node["id"] in targets
        ]
        ...
        for node in adjacent_targets:
            if node["id"] not in visited:
                ...
                layer_json["inbound_nodes"].append([current_node["id"], 0, 0, {}])
        ...
        if len(target_nodes[t]) > 1:
            starter_json["config"]["layers"].append(
                {
                    ...
                    "name": "concatenate" + str(i),
                    "inbound_nodes": [[[id, 0, 0, {}] for id in target_nodes[t]]],
                }
            )
            for node in starter_json["config"]["layers"]:
                if node["name"] == t:
                    ...
                    filtered_inbound = [
                        item for item in inbound_nodes if item[0] not in target_nodes[t]
                    ]
...
def helper_generate_layers(layer_params):
    if layer_params["type"] == "custominput":
        ...
        default_dense["config"]["activation"] = layer_params["data"]["params"][
            "activation"
        ]
        ...
        default_conv = {
            ...
            "config": {
                ...
                "kernel_size": (
                    int(layer_params["data"]["params"]["kernelX"]),
                    int(layer_params["data"]["params"]["kernelY"]),
                ),
                "strides": (
                    int(layer_params["data"]["params"]["strideX"]),
                    int(layer_params["data"]["params"]["strideY"]),
                ),
