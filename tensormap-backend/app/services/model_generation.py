...
def model_generation(model_params: dict) -> dict:
    ...
    try:
        logger.debug(
            ...
            len(model_params.get("nodes", [])),
            len(model_params.get("edges", [])),
        )
...
def _build_secure_layer(node: dict, input_tensor, server_registry: dict):
    ...
    for _key, config in server_registry.items():
    ...
    for key, value in params.items():
        if key in allowed_params:  # Security Check: Is this a permitted parameter?
            ...
            is_select_param = param_schema.get("type") == "select"
            has_options = "options" in param_schema
            has_invalid_option = has_options and value not in param_schema["options"]
            if is_select_param and has_invalid_option:
                raise ValueError(f"Invalid value '{value}' for param '{key}'. Allowed: {param_schema['options']}")
